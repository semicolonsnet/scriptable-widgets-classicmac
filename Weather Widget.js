...
// replace YOUR_API_KEY with the actual key you receive
  const API_KEY =  keys ? keys.OpenWeatherMap : ''
...
const MONO_FONT = 'Menlo-Regular'
//const GLOBAL_TEXT_COLOR = Color.white()

// best to use a monospace font for alignment
const GLOBAL_FONT = 'ChicagoFLF'
const GLOBAL_TEXT_COLOR = Color.white()
...
async function createWidget(data, widgetFamily='medium') {
  ...
  const fontSize = 12//widgetFamily=='small'?9:15
  const useFont = new Font(GLOBAL_FONT, fontSize)  
  const widget = new ListWidget()
  const imageName = "widgettop.PNG"
...
// 
// 
   const desc = cond.weather[0].description
    const condTxt = hstack.addText(desc)
    condTxt.font = useFont
    condTxt.textColor = GLOBAL_TEXT_COLOR
    
    hstack.addSpacer()

     const pop = `${Math.round(cond.pop * 100)}%`.padStart(4,' ')
    const chance = hstack.addText(pop)
    chance.leftAlignText()
    chance.font = useFont
